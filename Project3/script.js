...
function initialLoadHandler() {
  ...
  waccInput.value = 6.00;
}

function runButtonHandler() {
  const waccInput = document.getElementById('wacc');
  let wacc = waccInput.value / 200;// 2 for APR -> EffR, 100 for percent
  if (wacc >= 0) { 
    if (BONDCOUNTER > 1) {
      let dataArray = createDataArray();
      let npv = calcNPV(sumCF(dataArray), wacc);
      addResults(npv, wacc); 
    ...
    alert('Please use an interest rate of at least 0.');
  }
}
...
function validateInput(add) {
  ...
  if (errorString) {
    alert(errorString + '\n\nIf this is too hard for you, try using the random button to add a bond...');
  }
  return correctEntry;
}
...
function sumCF(dataArray) {
  // Goes through the dataArray created for the graph
  // Creates an array of all cashflows summed at each peroid
  let cfArray = [];
...
function discountArray(n, r) {
  // Creates an array of the discount factors for each time period
  arrDiscount = [];
  for (let t = 0; t < n; t++) {
...
// console.log(`Expect [1, 0.9709, 0.9426, 0.9254, 0.8885], got ${discountArray(5,.03)}`)


function calcNPV(arrBond, r) {
  // Takes a the array of CF's from sumCF
  // Does a sumproduct with a discount array to get NPV 
  let sum = 0;
  let arrDisc = discountArray(arrBond.length, r);
  for (i = 0; i < arrBond.length; i++) {
    // console.log(`r: ${r}, i: ${i}, cf ${arrBond[i]}, dc ${arrDisc[i]}`)
...
function getLongestMaturity() {
  // Before building the data array for chart, need to how long.
  let max = 0;
...
function addResults(npv, wacc) {
  npv = npv.toFixed(2);
  if (npv == 0) {
    npv *= -1;
  }
  let addHTML = document.getElementById("results");
  addHTML.style.padding = '10px';
  addHTML.textContent = `NPV: $${npv} with ${wacc * 200}% APR, 
  or ${(((1 + wacc) ** 2 - 1) * 100).toFixed(2)}% EAR.`;
}
